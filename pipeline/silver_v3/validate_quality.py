...
def normalized_token(token: dict[str, Any]) -> str:
    return str(token.get("normalized") or token.get("text") or "").strip().casefold()
...
def repeated_ngrams(rows: list[dict[str, Any]], n: int) -> tuple[int, list[dict[str, Any]]]:
    ...
    for index in range(max(0, len(tokens) - n + 1)):
        occurrences[tuple(tokens[index:index + n])].append(locations[index])
...
def immediate_duplicate_ngrams(rows: list[dict[str, Any]], n: int) -> tuple[int, list[dict[str, Any]]]:
    cases: list[dict[str, Any]] = []
    for row in rows:
        lexical = [
            (position, normalized_token(token))
            for position, token in enumerate(row.get("tokens", []))
            if normalized_token(token)
        ]
        for start in range(max(0, len(lexical) - (2 * n) + 1)):
            first = lexical[start:start + n]
            second = lexical[start + n:start + (2 * n)]
            if [value for _, value in first] != [value for _, value in second]:
                continue
            cases.append({
                "segment_position": int(row.get("segment_position", row.get("segment_index", 0))),
                "segment_id": row.get("segment_id"),
                "ngram": " ".join(value for _, value in first),
                "first_token_position": first[0][0],
                "second_token_position": second[0][0],
            })
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-repeated-6grams", type=int, default=1, help="Maximum immediate duplicate six-grams; non-adjacent repetition is telemetry only.")
    parser.add_argument("--pilot-segments", type=int, default=30)
    parser.add_argument("--pilot-min-a-tier-ratio", type=float, default=0.75)
    parser.add_argument("--pilot-max-repeated-6grams", type=int, default=1, help="Maximum immediate duplicate six-grams in pilot window.")
...
def main() -> int:
    ...
    full_immediate_count, full_immediate_cases = immediate_duplicate_ngrams(rows, 6)
    full["global_repeated_6gram_extra_occurrence_count"] = full_repeat_count
    full["immediate_duplicate_6gram_count"] = full_immediate_count
    ...
    pilot_immediate_count, pilot_immediate_cases = immediate_duplicate_ngrams(pilot_rows, 6)
    pilot["global_repeated_6gram_extra_occurrence_count"] = pilot_repeat_count
    pilot["immediate_duplicate_6gram_count"] = pilot_immediate_count
    ...
    required_input_views_pass = all(
        check["present"] and check["normalized_row_count"] > 0 and check["normalized_word_count"] > 0
        and check["source_accounting_closed"] and check["passed"]
        for check in view_input_checks.values()
    )
    contribution = full["view_contribution_counts"]
    anchor_contribution_pass = contribution.get("canonical_20s", 0) > 0 and contribution.get("whole_slice", 0) > 0
    ...
    gates = {
        ...
        "full_immediate_duplicate_6grams_pass": full_immediate_count <= args.max_repeated_6grams,
        "pilot_a_tier_ratio_pass": pilot["a_tier_ratio"] >= args.pilot_min_a_tier_ratio,
        "pilot_immediate_duplicate_6grams_pass": pilot_immediate_count <= args.pilot_max_repeated_6grams,
    ...
    report = {
        "schema_version": "silver_v3_quality_validation_v2",
        ...
        "thresholds": {
            ...
            "max_immediate_duplicate_6grams": args.max_repeated_6grams,
            "pilot_segments": args.pilot_segments,
            "pilot_min_a_tier_ratio": args.pilot_min_a_tier_ratio,
            "pilot_max_immediate_duplicate_6grams": args.pilot_max_repeated_6grams,
            "require_anchor_contribution": args.require_anchor_contribution,
        },
        "view_input_checks": view_input_checks,
        "full_lecture": full,
        "pilot_window": pilot,
        "full_immediate_duplicate_6grams": full_immediate_cases,
        "pilot_immediate_duplicate_6grams": pilot_immediate_cases,
        "top_full_global_repeated_6grams": full_repeats[:100],
        "top_pilot_global_repeated_6grams": pilot_repeats[:100],
        "gates": gates,
        "passed": passed,
    }
